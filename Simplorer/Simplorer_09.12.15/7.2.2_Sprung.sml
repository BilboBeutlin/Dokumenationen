...
INTERN  C   C1   N1:=N0054,  N2:=GND  ( C := 1.572u, V0 :=   ) ;
...
INTERN  EPULSE 	E1   N1:=GND,  N2:=N0053  ( FREQ := 0.01, TPERIO := 100, AMPL := -50, PHASE := 0, PERIO := 1, OFF := 0, TDELAY := 0.001, SPC := 0, AC_PHASE := 0, AC_MAG := 1  );
INTERN  AM  AM1   N1:=N0054,  N2:=N0013  ( ) ;
INTERN  C   C3   N1:=N0056,  N2:=GND  ( C := 5, V0 :=   ) ;
INTERN  R   R2   N1:=N0054,  N2:=N0053  ( R := 100  ) ;
INTERN L   L2  N1:= N0056,  N2:=N0033  ( L := 0.033u, I0 :=   ) ;
INTERN  R   R3   N1:=N0056,  N2:=N0033  ( R := 1  ) ;
INTERN  VM  VM3  N1:=N0056,  N2:=GND  ( ) ;
INTERN  INTG  INTG1   ( INPUT:=VM3.V,KI:=1,Y0:=0,UL:=0,LL:=0,TRIGSIG:=0,TRIGVAL:=1,CTRL:=0,TS:=0) ;
...
SIMCFG SIMPLORER_TR Simplorer1 ( Tend := 800m, Hmin := 10u, Hmax := 1m );
SIMCFG SIMPLORER_AC Simplorer2 ( Fstart := 100, Fend := 10k, Fstep := 100, ACSweepType := 1, EMaxAC := 1u );
...
RESULT VIEW VANALOG_41 (  AM1.I, Type:=ANALOG );
RESULT SDB SDB_42(  C3.V );
RESULT SDB SDB_43(  C3.I );
RESULT SDB SDB_44(  R2.V );
RESULT SDB SDB_45(  R2.I );
RESULT SDB SDB_46(  L2.V );
RESULT SDB SDB_47(  L2.I );
RESULT SDB SDB_48(  R3.V );
RESULT SDB SDB_49(  R3.I );
RESULT VIEW VANALOG_50 (  C3.V, Type:=ANALOG );
RESULT VIEW VANALOG_51 (  C3.I, Type:=ANALOG );
RESULT VIEW VANALOG_52 (  R2.V, Type:=ANALOG );
RESULT VIEW VANALOG_53 (  R2.I, Type:=ANALOG );
RESULT VIEW VANALOG_54 (  L2.V, Type:=ANALOG );
RESULT VIEW VANALOG_55 (  L2.I, Type:=ANALOG );
RESULT VIEW VANALOG_56 (  R3.V, Type:=ANALOG );
RESULT VIEW VANALOG_57 (  R3.I, Type:=ANALOG );
RESULT SDB SDB_58(  VM3.V );
RESULT VIEW VANALOG_59 (  VM3.V, Type:=ANALOG );
RESULT VIEW VANALOG_60 (  INTG1.VAL, Type:=ANALOG );
RESULT SDB SDB_61(  INTG1.VAL );
