...
INTERN  EPULSE 	E1   N1:=GND,  N2:=N0046  ( FREQ := 0.1, TPERIO := 10, AMPL := -50, PHASE := 0, PERIO := 1, OFF := 0, TDELAY := 0.0001, SPC := 0, AC_PHASE := 0, AC_MAG := 1  );
...
SIMCFG SIMPLORER_TR Simplorer1 ( Tend := 0.6, Hmin := 10u, Hmax := 1m );
...
RESULT VIEW VANALOG_48 (  C3.V, Type:=ANALOG );
RESULT VIEW VANALOG_49 (  C3.I, Type:=ANALOG );
